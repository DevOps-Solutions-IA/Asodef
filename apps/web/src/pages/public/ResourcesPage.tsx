...
export function ResourcesPage() { return <><Seo routeKey="resources" breadcrumbs={[{ name: "Inicio", path: "/" }, { name: "Recursos", path: "/recursos" }]}/><PublicHero eyebrow="Recursos" title={<>Accede al servicio que <span className="text-brand-orange">necesitas</span></>} description="Consulta pagos, radica casos, ejerce derechos sobre tus datos o encuentra una respuesta frecuente." actions={[{ label: "Recibir orientación", to: "/comenzar", primary: true }, { label: "Ver preguntas frecuentes", to: "/recursos/preguntas-frecuentes" }]}/><EditorialSection><SectionIntro eyebrow="Accesos directos" title="Elige una acción" description="Cada opción abre el proceso que recibe y gestiona ese tipo de solicitud."/><div className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{resources.map(({icon:Icon,...r})=><Link key={r.to} to={r.to} className="group rounded-3xl border border-brand-dark/10 bg-white p-7 shadow-e1 transition hover:-translate-y-1 hover:shadow-e3 motion-reduce:transform-none"><Icon className="h-7 w-7 text-brand-orange"/><h2 className="mt-7 font-display text-2xl font-semibold">{r.title}</h2><p className="mt-3 leading-7 text-text-muted">{r.text}</p><span className="mt-7 inline-flex items-center gap-2 font-semibold text-brand-dark">Abrir<ArrowRight className="h-4 w-4 transition group-hover:translate-x-1 motion-reduce:transform-none"/></span></Link>)}</div></EditorialSection><PageCta title="¿No sabes cuál proceso usar?" description="Indica tu necesidad para abrir el primer servicio aplicable." label="Recibir orientación"/></>; }

...
export function SolutionsPage() {
  return <><Seo routeKey="solutions" breadcrumbs={[{ name: "Inicio", path: "/" }, { name: "Soluciones", path: "/soluciones" }]} />
    <PublicHero eyebrow="Rutas por perfil" title={<>Una plataforma, recorridos distintos para cada <span className="text-brand-orange">relación</span></>} description="Las soluciones explican qué puede hacer cada audiencia, qué canal aplica y qué información se necesita. No duplican el portafolio: organizan la forma de usarlo." actions={[{ label: "Ayúdame a elegir", to: "/comenzar", primary: true }, { label: "Explorar beneficios", to: "/beneficios" }]} />
    <EditorialSection><SectionIntro eyebrow="Selecciona tu perfil" title="Empieza desde tu relación con ASODEF" description="Cada recorrido conecta módulos, beneficios, portales y consideraciones legales relevantes."/><div className="mt-12 grid gap-5 md:grid-cols-2">{AUDIENCES.map(audience=>{const Icon=icons[audience.slug]; return <Link key={audience.slug} to={`/soluciones/${audience.slug}`} className="group rounded-[2rem] border border-brand-dark/10 bg-white p-8 shadow-e1 transition hover:-translate-y-1 hover:shadow-e3"><Icon aria-hidden className="h-8 w-8 text-brand-orange"/><h2 className="mt-8 font-display text-3xl font-semibold">{audience.title}</h2><p className="mt-4 leading-7 text-text-muted">{audience.summary}</p><span className="mt-8 inline-flex items-center gap-2 font-semibold text-brand-dark">Ver recorrido<ArrowRight className="h-4 w-4 transition group-hover:translate-x-1"/></span></Link>})}</div></EditorialSection>
    ...
    <PageCta title="Si tu perfil no es evidente, usa el orientador" description="La primera respuesta clasifica la visita y evita enviarte a un formulario genérico." />
  </>;
}
...
export function AudiencePage({ fixedAudience }: { fixedAudience?: "empresas" } = {}) {
  ...
  return <><Seo custom={{ path: `/soluciones/${audience.slug}`, title: `${audience.title} | ASODEF`, description: audience.summary }} breadcrumbs={[{ name: "Inicio", path: "/" }, { name: "Soluciones", path: "/soluciones" }, { name: audience.title, path: `/soluciones/${audience.slug}` }]} faq={audience.faq}/>
    ...
    <EditorialSection><div className="grid gap-12 lg:grid-cols-[.75fr_1.25fr]"><SectionIntro eyebrow="Necesidades" title="Lo que esta ruta puede resolver" description="Solo incluye capacidades que existen en la plataforma o en el portafolio documentado."/><div className="surface-panel rounded-3xl p-8"><OutcomeList items={audience.needs}/></div></div></EditorialSection>
    <EditorialSection tone="soft"><SectionIntro eyebrow="Recorridos aplicables" title="De la necesidad al módulo correcto" description="Cada acción conduce a una ruta real y conserva las reglas de autenticación, permisos y consentimiento."/><div className="mt-12 grid gap-5 lg:grid-cols-3">{audience.workflows.map((flow,i)=><article key={flow.title} className="flex flex-col rounded-3xl bg-white p-7 shadow-e1"><span className="text-xs font-bold text-brand-orange">0{i+1}</span><h2 className="mt-7 font-display text-2xl font-semibold">{flow.title}</h2><p className="mt-4 flex-1 leading-7 text-text-muted">{flow.description}</p><Link className="mt-7 inline-flex items-center gap-2 font-semibold text-brand-dark" to={flow.to}>Ir a esta ruta<ArrowRight className="h-4 w-4"/></Link></article>)}</div></EditorialSection>
    ...
    <PageCta title={`Continúa con la ruta para ${audience.title.toLowerCase()}`} description="El recorrido guiado conserva tu perfil y pide únicamente la información necesaria para el siguiente paso." />
  </>;
}

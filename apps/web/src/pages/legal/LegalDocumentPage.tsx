...
export function LegalDocumentPage({ slug, title }: LegalDocumentPageProps) {
  ...
  return (
    <article className="legal-document-print">
      <header className="border-b border-border-soft pb-7">
      ...
      {data?.content && (
        <>
          <details className="print-hide mt-7 rounded-xl border border-border-soft bg-white p-4 shadow-e1 lg:hidden"><summary className="cursor-pointer font-display text-sm font-semibold text-brand-dark">Contenido del documento</summary><ol className="mt-3 flex flex-col gap-2 text-sm text-text-muted">{sections.map((section, sectionIndex) => <li key={section.id}><a href={`#${section.id}`} className="hover:text-brand-dark">{sectionIndex + 1}. {section.heading}</a></li>)}</ol></details>
          <div className="mt-8 grid items-start gap-10 lg:grid-cols-[210px_minmax(0,1fr)]">
            <aside className="print-hide sticky top-6 hidden rounded-2xl border border-border-soft bg-white p-4 shadow-e1 lg:block"><p className="font-display text-sm font-semibold text-brand-dark">En este documento</p><ol className="mt-3 flex max-h-[calc(100vh-9rem)] flex-col gap-2 overflow-y-auto pr-1 text-xs leading-5 text-text-muted">{sections.map((section, sectionIndex) => <li key={section.id}><a href={`#${section.id}`} className="block rounded-md px-2 py-1 hover:bg-brand-dark-50 hover:text-brand-dark">{sectionIndex + 1}. {section.heading}</a></li>)}</ol></aside>

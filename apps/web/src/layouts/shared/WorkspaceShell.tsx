...
import { ChevronDown, Menu, ShieldCheck, type LucideIcon } from "lucide-react";
    ...
    >
      <div className="flex flex-col gap-5">
        {groups.map((group) => (
          <section
            key={group.label}
            aria-labelledby={`workspace-nav-${group.label.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`}
          >
            <h2
              id={`workspace-nav-${group.label.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`}
              className={cn(
                "px-3 pb-1.5 text-[10px] font-bold uppercase tracking-[0.16em]",
                tone === "dark" ? "text-white/45" : "text-text-muted",
              )}
            >
              {group.label}
            </h2>
...
export function WorkspaceShell({
  ...
  const visibleGroups = (navGroups ?? [{ label: navLabel, items: navItems }])
    .map((group) => ({
      ...group,
  ...
  const brandBlock = (
    <div
      className={cn(
        "border-b px-5 py-5",
        dark ? "border-white/10" : "border-brand-dark/10",
      )}
    >
      <Link
        to="/"
        aria-label={ASODEF_COMPANY.legalName}
        className="inline-flex"
      >
        <BrandLogo
          variant={dark ? "icon" : "full"}
          className={cn("w-auto", dark ? "h-10" : "h-9")}
        />
      </Link>
      <div className="mt-4 flex items-center gap-2">
        <span
          className={cn(
            "h-px w-7",
            dark ? "bg-brand-orange" : "bg-brand-orange",
          )}
          aria-hidden="true"
        />
        <p
    ...
    <div className="workspace-canvas min-h-screen lg:flex">
      ...
      <aside
        className={cn(
          ...
          dark
            ? "border-white/10 bg-brand-deep text-white"
            : "border-brand-dark/10 bg-white/92 backdrop-blur-xl",
        )}
      >
        {brandBlock}
        <p
          className={cn(
            "px-6 pt-5 text-[10px] font-bold uppercase tracking-[0.16em]",
        ...
        >
          {footerLinks && (
            <div
              className={cn(
                "mb-3 flex flex-col gap-1 px-2 text-xs",
                dark ? "text-white/60" : "text-text-muted",
              )}
            >
              {footerLinks}
      ...
      <div className="min-w-0 flex-1">
          ...
          <div className="flex h-[var(--workspace-topbar-height)] items-center justify-between gap-3 px-4 sm:px-6 lg:px-8">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="truncate font-display text-base font-semibold text-brand-dark sm:text-lg">
                  {productLabel}
                </p>
            ...
            <div className="relative">
              <button
                ...
                className="flex min-h-11 shrink-0 items-center gap-3 rounded-full border border-brand-dark/10 bg-white/80 py-1.5 pl-1.5 pr-3 text-left shadow-e1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-orange focus-visible:ring-offset-2"
              >
                ...
                <span className="hidden max-w-48 sm:block">
                  <span className="block truncate text-xs font-semibold text-text-main">
                    {user?.fullName ?? "Usuario ASODEF"}
                  </span>
                  <span className="block truncate text-[10px] text-text-muted">
                    {user?.roles.join(" · ") || "Sesión activa"}
                  </span>
                </span>
                <ChevronDown
                  aria-hidden="true"
              ...
              {accountOpen && (
                <div
                  ...
                  className="absolute right-0 top-[calc(100%+0.5rem)] z-50 w-64 rounded-2xl border border-brand-dark/10 bg-white p-2 shadow-e4"
                >
                  <p className="px-3 py-2 text-[10px] font-bold uppercase tracking-[0.16em] text-text-muted">
                    Mi cuenta
                  </p>
                  {visibleAccountItems.map((item) => {
                    ...
                    return (
                      <Link
                        ...
                        className="flex min-h-11 items-center gap-3 rounded-xl px-3 py-2 text-sm font-medium text-text-muted hover:bg-brand-dark-50 hover:text-brand-dark focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-orange"
                      >
                  ...
                  <div className="mt-1 border-t border-brand-dark/10 pt-1">
        ...
        <main
          ...
          ref={mainRef}
          tabIndex={-1}
          className="workspace-main min-w-0 px-4 py-6 focus:outline-none sm:px-6 sm:py-8 lg:px-8 xl:px-10"
        >

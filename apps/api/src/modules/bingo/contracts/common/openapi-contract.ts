...
const admin = BINGO_ADMIN_ROUTE_CONTRACTS.map((route, index): BingoOpenApiOperationContract => ({
  operationId: `bingoAdminOperation${index + 1}`,
  method: route.method,
  path: route.path,
  surface: "ADMIN",
  security: "ADMIN_COOKIE",
  responseContract: route.path.includes("audit") ? "AdminBingoAuditContract" : "BingoAdminCommandResultContract",
}));
...
const affiliate = BINGO_AFFILIATE_ROUTE_CONTRACTS.map((route, index): BingoOpenApiOperationContract => ({
...
const publicResponses = ["PublicBingoEventContract", "PublicBingoSnapshotContract"] as const;
const publicOperations = BINGO_PUBLIC_ROUTE_CONTRACTS.map((route, index): BingoOpenApiOperationContract => ({
...
export const BINGO_OPENAPI_V1_OPERATIONS = Object.freeze([...admin, ...affiliate, ...publicOperations]);

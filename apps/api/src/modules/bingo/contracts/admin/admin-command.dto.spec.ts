...
async function errors<T extends object>(type: new () => T, body: unknown) {
  return validate(plainToInstance(type, body), { whitelist: true, forbidNonWhitelisted: true });
}

describe("Bingo admin API contracts", () => {
  it("accepts a complete, bounded event command", async () => {
    await expect(errors(CreateBingoEventDto, {
  ...
  it("rejects mass-assigned event lifecycle and actor fields", async () => {
    ...
    expect(result.map(({ property }) => property)).toEqual(expect.arrayContaining(["status", "actorUserId"]));
  });
  ...
  it("rejects malformed and oversized custom masks", async () => {
    ...
    expect(plainToInstance(BingoPatternMaskDto, { positions: [0, 12, 24] })).toBeInstanceOf(BingoPatternMaskDto);
  });
  ...
  it("does not accept identity lookup by PII", async () => {
    ...
    expect(result.map(({ property }) => property)).toEqual(expect.arrayContaining(["documentNumber", "phone"]));
  });

  it("enforces a single referentially meaningful participant identity shape", async () => {
    await expect(errors(RegisterBingoParticipantDto, { kind: "AFFILIATE", affiliateId: "9f9f60cb-92b4-4d76-a9cf-b60dfe6613fa" })).resolves.toHaveLength(0);
    await expect(errors(RegisterBingoParticipantDto, {
      kind: "AFFILIATE",
      affiliateId: "9f9f60cb-92b4-4d76-a9cf-b60dfe6613fa",
      authorizedSubjectRef: "ambiguous",
    })).resolves.not.toHaveLength(0);
    await expect(errors(RegisterBingoParticipantDto, { kind: "PARTNER_COMPANY_MEMBER", companyId: "9f9f60cb-92b4-4d76-a9cf-b60dfe6613fa" })).resolves.not.toHaveLength(0);
  });
  ...
  it("requires idempotency on every declared mutation and maps concrete RBAC permissions", () => {
    for (const route of BINGO_ADMIN_ROUTE_CONTRACTS) {
      ...
      expect(route.mutation ? route.requiresIdempotencyKey : !route.requiresIdempotencyKey).toBe(true);
    }
  });
});

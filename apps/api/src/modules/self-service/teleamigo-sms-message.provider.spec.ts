...
const FIXTURE_KEY = ["fixture", "value", "abcdefghijklmnopqrstuvwxyz"].join("-");
...
function config(): ConfigService<EnvConfig, true> {
  const values: Partial<EnvConfig> = {
    ...
    TELEAMIGO_SMS_API_KEY: FIXTURE_KEY,
...
describe("TeleamigoSmsMessageProvider", () => {
  ...
  it("sends the ASODEF OTP through the verified SMS v3 transport", async () => {
  ...
  it("does not claim delivery when the provider rejects the message", async () => {

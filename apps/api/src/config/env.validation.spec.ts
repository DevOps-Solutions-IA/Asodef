...
const VALID_ENV: Record<string, string> = {
  ...
  DATABASE_URL:
    "postgresql://asodef:asodef_dev_password@localhost:5433/asodef?schema=public",
  ...
  CONTRACT_DOWNLOAD_TOKEN_SECRET:
    "a-contract-download-secret-that-is-long-enough",
};
...
describe("validateEnv", () => {
  it("boots with a minimal valid configuration and fills in documented defaults", () => {
    ...
    expect(result.AI_RUNTIME_ENABLED).toBe(false);
    expect(result.OPENROUTER_API_KEY).toBe("");
    expect(result.OPENROUTER_MAX_ATTEMPTS).toBe(2);
    expect(result.DATABASE_URL).toBe(VALID_ENV.DATABASE_URL);
  });

  it("requires an injected OpenRouter credential only when AI runtime is enabled", () => {
    expect(validateEnv(VALID_ENV).AI_RUNTIME_ENABLED).toBe(false);
    expect(() =>
      validateEnv({ ...VALID_ENV, AI_RUNTIME_ENABLED: "true" }),
    ).toThrow(/OPENROUTER_API_KEY/);
    const enabled = validateEnv({
      ...VALID_ENV,
      AI_RUNTIME_ENABLED: "true",
      OPENROUTER_API_KEY: "ci-placeholder-not-a-real-provider-key",
    });
    expect(enabled.AI_RUNTIME_ENABLED).toBe(true);
  });

  it("pins OpenRouter transport to HTTPS and bounds retry policy", () => {
    expect(() =>
      validateEnv({
        ...VALID_ENV,
    ...
    expect(() =>
      validateEnv({ ...VALID_ENV, OPENROUTER_MAX_ATTEMPTS: "4" }),
    ).toThrow(/OPENROUTER_MAX_ATTEMPTS/);
  });
  ...
  it("parses staged MFA enforcement and bounds its security TTLs", () => {
    expect(
      validateEnv({ ...VALID_ENV, ADMIN_MFA_REQUIRED: "true" })
        .ADMIN_MFA_REQUIRED,
    ).toBe(true);
    expect(() =>
      validateEnv({ ...VALID_ENV, ADMIN_MFA_CHALLENGE_TTL_SECONDS: "30" }),
    ).toThrow();
    expect(() =>
      validateEnv({ ...VALID_ENV, ADMIN_MFA_ENROLLMENT_TTL_SECONDS: "7200" }),
    ).toThrow();
    expect(() =>
      validateEnv({ ...VALID_ENV, ADMIN_STEP_UP_TTL_SECONDS: "3600" }),
    ).toThrow();
    expect(() =>
      validateEnv({ ...VALID_ENV, ADMIN_STEP_UP_MAX_FAILED_ATTEMPTS: "2" }),
    ).toThrow();
    expect(() =>
      validateEnv({ ...VALID_ENV, ADMIN_STEP_UP_MAX_FAILED_ATTEMPTS: "11" }),
    ).toThrow();
    expect(() =>
      validateEnv({
        ...VALID_ENV,
        ADMIN_STEP_UP_RATE_LIMIT_WINDOW_SECONDS: "30",
      }),
    ).toThrow();
    expect(() =>
      validateEnv({
        ...VALID_ENV,
        ADMIN_STEP_UP_RATE_LIMIT_WINDOW_SECONDS: "7200",
      }),
    ).toThrow();
  });
  ...
  it("accepts a completely empty SMTP block and fails closed on partial SMTP configuration", () => {
    expect(
      validateEnv({ ...VALID_ENV, SMTP_PORT: "" }).SMTP_PORT,
    ).toBeUndefined();
    expect(() =>
      validateEnv({
        ...VALID_ENV,
    ...
    expect(() =>
      validateEnv({ ...VALID_ENV, SMTP_FROM: "no-reply@asodef.com.co" }),
    ).toThrow(/SMTP_HOST/);
    expect(() =>
      validateEnv({
        ...VALID_ENV,
  ...
  it("requires an SMTP FQDN suitable for certificate hostname verification", () => {
    expect(() =>
      validateEnv({ ...VALID_ENV, SMTP_HOST: "smtp://smtp.asodef.com.co" }),
    ).toThrow(/SMTP_HOST/);
    expect(() => validateEnv({ ...VALID_ENV, SMTP_HOST: "localhost" })).toThrow(
      /SMTP_HOST/,
    );
    expect(() => validateEnv({ ...VALID_ENV, SMTP_HOST: "127.0.0.1" })).toThrow(
      /SMTP_HOST/,
    );
    expect(() =>
      validateEnv({ ...VALID_ENV, SMTP_CONNECT_HOST: "smtp://172.25.52.1" }),
    ).toThrow(/SMTP_CONNECT_HOST/);
  });

  it("rejects an invalid SMTP sender address without echoing credentials", () => {
    expect(() =>
      validateEnv({ ...VALID_ENV, SMTP_FROM: "not-an-email" }),
    ).toThrow(/SMTP_FROM/);
  });
  ...
  it("requires every Firebird connection field only when the feature is enabled", () => {
    ...
    ).toThrow(/MASTER_FIREBIRD_HOST/);
    expect(() =>
      validateEnv({ ...VALID_ENV, MASTER_FIREBIRD_ENABLED: "true" }),
    ).toThrow(/MASTER_FIREBIRD_DATABASE/);
    expect(() =>
      validateEnv({ ...VALID_ENV, MASTER_FIREBIRD_ENABLED: "true" }),
    ).toThrow(/MASTER_FIREBIRD_USER/);
    expect(() =>
      validateEnv({ ...VALID_ENV, MASTER_FIREBIRD_ENABLED: "true" }),
    ).toThrow(/MASTER_FIREBIRD_PASSWORD/);
  });
  ...
  it("never echoes the invalid value itself when DATABASE_URL has the wrong shape", () => {
    const secretLookingValue =
      "not-a-postgres-url-but-looks-like-a-secret-abc123";
    try {
  ...
  it("rejects a JWT_REFRESH_SECRET that is too short", () => {
    expect(() =>
      validateEnv({ ...VALID_ENV, JWT_REFRESH_SECRET: "short" }),
    ).toThrow(/JWT_REFRESH_SECRET/);
  });

  it("rejects an ENCRYPTION_KEY shorter than 32 characters", () => {
    expect(() =>
      validateEnv({ ...VALID_ENV, ENCRYPTION_KEY: "too-short" }),
    ).toThrow(/ENCRYPTION_KEY/);
  });

  it("rejects a zero or negative LOGIN_MAX_FAILED_ATTEMPTS", () => {
    expect(() =>
      validateEnv({ ...VALID_ENV, LOGIN_MAX_FAILED_ATTEMPTS: "0" }),
    ).toThrow(/LOGIN_MAX_FAILED_ATTEMPTS/);
    expect(() =>
      validateEnv({ ...VALID_ENV, LOGIN_MAX_FAILED_ATTEMPTS: "-1" }),
    ).toThrow(/LOGIN_MAX_FAILED_ATTEMPTS/);
  });

  it("rejects an excessive LOGIN_MAX_FAILED_ATTEMPTS that would defeat the control", () => {
    expect(() =>
      validateEnv({ ...VALID_ENV, LOGIN_MAX_FAILED_ATTEMPTS: "100000" }),
    ).toThrow(/LOGIN_MAX_FAILED_ATTEMPTS/);
  });

  it("rejects a zero or negative LOGIN_LOCKOUT_DURATION_MINUTES", () => {
    expect(() =>
      validateEnv({ ...VALID_ENV, LOGIN_LOCKOUT_DURATION_MINUTES: "0" }),
    ).toThrow(/LOGIN_LOCKOUT_DURATION_MINUTES/);
  });

  it("rejects an excessive LOGIN_LOCKOUT_DURATION_MINUTES that could make accounts effectively permanently inaccessible", () => {
    expect(() =>
      validateEnv({ ...VALID_ENV, LOGIN_LOCKOUT_DURATION_MINUTES: "999999" }),
    ).toThrow(/LOGIN_LOCKOUT_DURATION_MINUTES/);
  });

  it("rejects an invalid (zero/negative/excessive) LOGIN_RATE_LIMIT_WINDOW_SECONDS", () => {
    expect(() =>
      validateEnv({ ...VALID_ENV, LOGIN_RATE_LIMIT_WINDOW_SECONDS: "0" }),
    ).toThrow(/LOGIN_RATE_LIMIT_WINDOW_SECONDS/);
    expect(() =>
      validateEnv({
        ...VALID_ENV,
        LOGIN_RATE_LIMIT_WINDOW_SECONDS: "999999999",
      }),
    ).toThrow(/LOGIN_RATE_LIMIT_WINDOW_SECONDS/);
  });

  it("fails fast when PASSWORD_RESET_TOKEN_SECRET is missing", () => {
    const { PASSWORD_RESET_TOKEN_SECRET: _unused, ...withoutResetSecret } =
      VALID_ENV;
    void _unused;

    expect(() => validateEnv(withoutResetSecret)).toThrow(
  ...
  it("rejects a PASSWORD_RESET_TOKEN_SECRET that is too short", () => {
    expect(() =>
      validateEnv({ ...VALID_ENV, PASSWORD_RESET_TOKEN_SECRET: "short" }),
    ).toThrow(/PASSWORD_RESET_TOKEN_SECRET/);
  });

  it("rejects an unrecognized NODE_ENV value", () => {
    expect(() =>
      validateEnv({ ...VALID_ENV, NODE_ENV: "staging-typo" }),
    ).toThrow(/NODE_ENV/);
  });

  it("rejects a non-numeric API_PORT", () => {
    expect(() => validateEnv({ ...VALID_ENV, API_PORT: "not-a-port" })).toThrow(
      /API_PORT/,
    );
  });
  ...
  it("coerces PRODUCTION_PAYMENTS_ENABLED from the string 'true' to a real boolean", () => {
    const result = validateEnv({
      ...VALID_ENV,
      PRODUCTION_PAYMENTS_ENABLED: "true",
    });

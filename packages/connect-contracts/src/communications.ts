...
> = {
  ...
  idempotency: {
    required: true,
    scope:
      "GatewayRequestContext.identity.effectiveActorId + idempotencyKey",
    duplicateBehavior:
...
export interface CommunicationsServiceContract {
  send(
    ...
    /** Canonical trusted context. It is created by Tool Gateway or an
     * authorized system runner and never accepted from LLM tool arguments. */
...
export interface CommunicationsSendToolBinding
  extends Omit<GovernedToolContract, "execution"> {
  operation: "communications.send";
  contractVersion: typeof COMMUNICATIONS_SEND_CONTRACT.version;
...
export const COMMUNICATIONS_SEND_TOOL_BINDING = Object.freeze({
  ...
  description:
    "Queues a governed, consent-checked communication request without exposing its delivery transport.",

...
| Automations                                                    | `@asodef/connect-contracts/automation`                                                   | `BACKEND_RUNTIME_MISSING` | Trigger, conditions, actions, version, execution/history and dead-letter contracts are integrated. Execution/admin runtime is absent.     |
| Communications and Templates                                   | `@asodef/connect-contracts/communications` and `templates`                               | `BACKEND_RUNTIME_MISSING` | Send/preview/lifecycle contracts are integrated. All canonical transports remain `CONTRACT_ONLY`; no admin or send runtime is verified.   |
...
- Automation contracts are `IN_MAIN`; execution and admin runtimes are `NOT_CONFIGURED`.
- Communications contracts are `IN_MAIN`; every transport, including EMAIL, is `CONTRACT_ONLY`, so sending is `NOT_CONFIGURED`.

...
test.describe("admin information architecture", () => {
  ...
  test.beforeAll(async ({ browser }) => {
    await ensureTestActor(
      PRIVILEGED_TEST_EMAIL,
      "E2E Information Architecture Administrator",
      "SUPER_ADMIN",
    );
  ...
  for (const viewport of VIEWPORTS) {
    test(`${viewport.name}: business Dashboard, System and Mi cuenta remain reachable and responsive`, async ({
      browser,
    }) => {
      ...
      try {
        ...
        for (const technicalLabel of [
          ...
          await expect(
            page.getByText(technicalLabel, { exact: true }),
          ).toHaveCount(0);
        }
        ...
        if (viewport.name === "desktop") {
          ...
          for (const group of [
            ...
            await expect(
              nav.getByRole("heading", { name: group }),
            ).toBeVisible();
        } else {
          await page.getByRole("button", { name: "Abrir navegación" }).click();
          await expect(
            page.getByRole("navigation", { name: "Administración" }).last(),
          ).toBeVisible();
          await page.keyboard.press("Escape");
        }

        await page
          .getByRole("button", { name: "Abrir menú de Mi cuenta" })
          .click();
        await expect(
        ...
        await expect(
          page.getByRole("heading", { name: "Seguridad de mi cuenta" }),
        ).toBeVisible();

        await page.goto("/admin/sistema");
        await expect(
          page.getByRole("heading", { name: "Sistema" }),
        ).toBeVisible();
        await page.getByRole("button", { name: "Integraciones" }).click();
        await expect(
          page.getByRole("heading", { name: "Integraciones externas" }),
        ).toBeVisible();
        await expect(
          page.getByRole("heading", { name: "Master / Firebird" }),
        ).toBeVisible();
    ...
    test(`${viewport.name}: Control Plane foundations remain separated, accessible and responsive`, async ({
      browser,
    }) => {
      const { context, page } = await openAuthenticatedPage(browser);
      await page.setViewportSize(viewport);
      try {
        ...
        await expect(
          page.getByRole("heading", { name: "Planes" }),
        ).toBeVisible();
        await expect(
        ...
        await expect(
          page.getByRole("heading", { name: "Plantillas" }),
        ).toBeVisible();
        await expect(
          page.getByText("Vista previa", { exact: true }),
        ).toBeVisible();
        await expect(
          page.getByText("Diferencias", { exact: true }),
        ).toBeVisible();

        await page.goto("/admin/sistema?section=proveedores");
        await expect(
          page.getByRole("heading", { name: "Salud de proveedores" }),
        ).toBeVisible();
        await expect(
          page.getByRole("heading", { name: "OpenRouter" }),
        ).toBeVisible();
        await expect(
          page.getByText("Conexión", { exact: true }).first(),
        ).toBeVisible();
        ...
        if (viewport.name === "desktop") {
          const nav = page.getByRole("navigation", { name: "Administración" });
          await expect(
            nav.getByRole("heading", { name: "Koral" }),
          ).toBeVisible();
          await expect(
            nav.getByRole("heading", { name: "Comunicaciones" }),
          ).toBeVisible();
        } else {
          await page.getByRole("button", { name: "Abrir navegación" }).click();
          await expect(
  ...
  test("legacy personal-security URLs redirect to the canonical Mi cuenta routes", async ({
    browser,
  }) => {
    const { context, page } = await openAuthenticatedPage(browser);
    try {
